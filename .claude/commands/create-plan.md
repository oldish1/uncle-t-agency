...
Create a detailed implementation plan for changes to this workspace. Plans are thorough documents that capture the full context, rationale, and step-by-step tasks needed to execute a change with complete alignment.

## Variables

request: $ARGUMENTS (describe what you want to plan, new integration, new command, new workflow, structural change, etc.)
...
- Create the plan in the `plans/` directory with filename: `YYYY-MM-DD-{descriptive-name}.md`
  - Use today's date
  - Replace `{descriptive-name}` with a short, kebab-case name (e.g., "add-stripe-integration", "build-email-automation", "create-crm-skill")
...
1. **Read core reference files:**
   - `CLAUDE.md`: workspace overview, structure, and conventions
   - `context/` files, background context on the user and their business
...
4. **Classify what you're building:**
   - Is this a Skill (auto-loads when relevant, has supporting files) or a Command (manual invocation, single prompt file)?
   - Skills live in `.claude/skills/{name}/SKILL.md` with a `description:` frontmatter for auto-loading
...
**Purpose:** <what this is for, what outcome does it enable>
...
<Connect this change to the user's goals. How does this add value?>
...
**Classification:** <Skill / Command / Script / N/A>
**Reasoning:** <Why this type was chosen>
**Location:** <File paths where this will live>
**Auto-discovery:** <For skills: description and trigger keywords>
...
1. The file path
2. A 2-sentence summary of the plan
3. Recommended next step: "Run `/implement plans/{filename}` when ready to execute."
